...
public actor AudD {
    ...
    /// Convenience overload; defers to `recognize(_:return:market:)` with `.url(...)`.
    @discardableResult
    public func recognize(
        ...
        return try await recognize(.url(url), return: `return`, market: market)
    }
    ...
    ///   - return: optional list of metadata sources to fetch (apple_music,
    ...
    public func recognize(
        ...
        let returnString = formatReturn(`return`)
    ...
    public var customCatalog: CustomCatalog {
        CustomCatalog(http: httpStandard, apiBase: apiBase, mutatingPolicy: mutatingPolicy())
    }
